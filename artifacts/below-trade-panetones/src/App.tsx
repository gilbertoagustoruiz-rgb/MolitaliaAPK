...
import { Camera, Check, CheckCircle2, Clock3, Download, FileSpreadsheet, Gift, LogOut, MapPin, Moon, PackageCheck, Pencil, Plus, RefreshCw, Search, ShieldCheck, ShoppingBag, Smartphone, Store, Sun, Trash2, Upload, UserRound, Users, Wifi, WifiOff, X } from 'lucide-react';
...
function Shell({ user, logout, theme, toggleTheme, children }: { user: AppUser; logout: () => void; theme: 'light' | 'dark'; toggleTheme: () => void; children: ReactNode }) {
  ...
  return <div className="bt-app"><header className="topbar"><Logo compact /><div className="campaign"><small>Campaña</small><strong>Panetones Molitalia</strong></div><div className="top-user"><Btn variant="ghost" className="theme-toggle" onClick={toggleTheme} testId="button-toggle-theme">{theme === 'dark' ? <Sun /> : <Moon />}<span>{theme === 'dark' ? 'Día' : 'Noche'}</span></Btn><div className="top-user-info"><strong>{user.name}</strong><small>{user.roleLabel || user.role}</small></div><Btn variant="ghost" onClick={logout} testId="button-logout"><LogOut /> Salir</Btn></div></header>{!online && <div className="offline-bar" role="status" aria-live="polite" data-testid="status-offline"><WifiOff /> Sin conexión · cambios guardados en este dispositivo</div>}{online && <div className="offline-bar" role="status" aria-live="polite" data-testid="status-online"><Wifi /> Con conexión · aplicación lista para operar</div>}{children}</div>;
}
...
function NewCanjeModal({ users, user, onSave, close }: { users: AppUser[]; user: AppUser; onSave: (canjes: InventoryMovement[]) => void | Promise<void>; close: () => void }) {
  ...
  return <Modal title="Abastecer stock operativo" detail="Selecciona un promotor e ingresa en una sola operación el Panetón para degustación y los artículos de canje. Deja en 0 lo que no corresponda." close={close}><div className="form-grid"><SelectField label="Promotor *" value={promoterId} onChange={setPromoterId} items={promoters.map(current => ({ value: current.id, label: `${current.name} · DNI ${current.dni}` }))} placeholder="Seleccionar promotor" /><Field label="Panetón para degustación"><Input type="number" value={tastingQuantity} onChange={setTastingQuantity} min={0} step={1} placeholder="0" testId="input-new-degustacion-quantity" /></Field>{redemptionItems.map(item => <Field key={item.id} label={`${item.label} recibida`}><Input type="number" value={quantities[item.id]} onChange={value => setQuantities(current => ({ ...current, [item.id]: value }))} min={0} step={1} placeholder="0" testId={`input-new-canje-${item.id.toLowerCase()}`} /></Field>)}</div>{selectedPromoter && <p className="modal-hint">Stock actual: {userTastingStock(selectedPromoter)} panetones · {redemptionStockText(userRedemptionStock(selectedPromoter))}</p>}{hasInvalidQuantity && <p className="modal-error">Las cantidades deben ser números enteros iguales o mayores que cero.</p>}<div className="modal-actions"><Btn variant="outline" onClick={close}>Cancelar</Btn><Btn disabled={!selectedPromoter || hasInvalidQuantity || !hasQuantity} onClick={save} testId="button-create-canje">Cargar stock</Btn></div></Modal>;
}
function AdminCanjesModule({ canjes, marketMap, users, onCreate, onDelete, onCleanup, canCleanup, notify }: { canjes: AdminCanje[]; marketMap: Record<string, Market>; users: AppUser[]; onCreate: () => void; onDelete: (canje: AdminCanje) => void; onCleanup: () => void; canCleanup: boolean; notify: (message: string, error?: boolean) => void }) {
  ...
  const visibleCanjes = canjes.filter(canje => `${canje.canjeId} ${canje.actorName} ${marketMap[canje.marketId]?.name || ''} ${canjeProductLabel(canje.canjeProductId, canje.canjeProductLabel || canje.degustacionProductLabel || canje.sale?.bonus || (canje.itemId ? redemptionLabel(canje.itemId) : undefined))}`.toLowerCase().includes(search.toLowerCase()));
  const exportCanjes = () => {
  ...
  return <section className="admin-module">
    ...
    {canCleanup && <div className="admin-warning"><Trash2 /><span><strong>Limpieza inicial</strong><small>Elimina todos los datos operativos y todos los usuarios excepto ANALISTA. Los precios se conservan.</small></span><Btn variant="danger" onClick={onCleanup} testId="button-cleanup-catalogs">Empezar desde cero</Btn></div>}
     <section className="panel"><div className="panel-header"><div><h2>{canjes.length} movimientos registrados</h2><p>Eliminar un abastecimiento revierte el stock cargado.</p></div></div><div className="panel-body sales-panel-body"><div className="sales-console-search"><Search /><Input value={search} onChange={setSearch} placeholder="Buscar código, producto, responsable o mercado" /></div>{visibleCanjes.length ? <div className="module-table-wrap"><div className="module-table"><div className="module-table-row module-table-header cols-7"><span>Código</span><span>Producto</span><span>Responsable</span><span>Origen</span><span>Cantidad</span><span>Fecha</span><span>Acciones</span></div>{visibleCanjes.map(canje => { const promoter = canje.promoterId ? users.find(current => current.id === canje.promoterId) : undefined; return <article className="module-table-row cols-7" key={`${canje.source}-${canje.canjeId}`}><span><b className="module-table-id">{canje.canjeId}</b><small>{canje.status}</small></span><span><strong>{canjeProductLabel(canje.canjeProductId, canje.canjeProductLabel || canje.degustacionProductLabel || canje.sale?.bonus || (canje.itemId ? redemptionLabel(canje.itemId) : undefined))}</strong><small>{marketMap[canje.marketId]?.name || 'Mercado no identificado'}</small></span><span><strong>{promoter?.name || canje.actorName}</strong><small>{promoter?.dni ? `DNI ${promoter.dni}` : canje.actorName}</small></span><span><strong>{canje.source === 'sale' ? 'Venta' : 'Abastecimiento'}</strong><small>{canje.kind}</small></span><span><strong>{canje.quantity}</strong><small>unidades</small></span><span><strong>{formatDate(canje.date)}</strong></span><span className="module-table-actions"><Btn variant="danger" onClick={() => onDelete(canje)} testId={`button-delete-canje-${canje.canjeId}`}><Trash2 /> Eliminar</Btn></span></article>; })}</div></div> : <Empty title={canjes.length ? 'No hay coincidencias' : 'Aún no hay movimientos'} detail="Abastece el stock personal de un promotor." />}</div></section>
  </section>;
}

function AdminDegustacionesModule({ consumos, users, marketMap, onDeleteConsumo }: { consumos: InventoryMovement[]; users: AppUser[]; marketMap: Record<string, Market>; onDeleteConsumo: (consumo: InventoryMovement) => void }) {
  const [search, setSearch] = useState('');
  const visibleConsumos = consumos.filter(consumo => { const promoter = users.find(current => current.id === (consumo.promoterId || consumo.actorId)); return `${consumo.id} ${promoter?.name || consumo.actorName} ${promoter?.dni || ''} ${marketMap[consumo.marketId]?.name || ''}`.toLowerCase().includes(search.toLowerCase()); });
  return <section className="admin-module">
    ...
    <section className="panel"><div className="panel-header"><div><h2>{consumos.length} consumos registrados</h2><p>Ordenados desde el más reciente hasta el más antiguo.</p></div></div><div className="panel-body sales-panel-body"><div className="sales-console-search"><Search /><Input value={search} onChange={setSearch} placeholder="Buscar código, promotor o mercado" /></div>{visibleConsumos.length ? <div className="module-table-wrap"><div className="module-table"><div className="module-table-row module-table-header cols-7"><span>Código</span><span>Promotor</span><span>Mercado</span><span>Ubicación</span><span>Cantidad</span><span>Fecha y estado</span><span>Acciones</span></div>{visibleConsumos.map(consumo => {
      ...
      return <article className="module-table-row cols-7" key={consumo.id}>
        <span><b className="module-table-id">{consumo.id}</b></span><span><strong>{promoter?.name || consumo.actorName || 'Promotor no identificado'}</strong><small>DNI {promoter?.dni || '—'} · {promoter?.roleLabel || promoter?.role || 'Promotor'}</small></span><span><strong>{market?.name || 'Mercado no identificado'}</strong></span><span><strong>{market?.district || '—'}</strong><small>{market ? `${market.province} · ${market.department}` : consumo.marketId}</small></span><span><strong>{consumo.quantity}</strong><small>unidades utilizadas</small></span><span><strong>{formatDate(consumo.date)}</strong><small>{consumo.status}</small></span><span className="module-table-actions"><Btn variant="danger" onClick={() => onDeleteConsumo(consumo)} testId={`button-delete-tasting-consumption-${consumo.id}`}><Trash2 /> Eliminar</Btn></span>
      </article>;
...
function CoordinatorApp({ user, markets, users, clients, sales, inventory, movements, assignments, notify }: { user: AppUser; markets: Market[]; users: AppUser[]; clients: Client[]; sales: Sale[]; inventory: MarketInventory[]; movements: InventoryMovement[]; assignments: PromoterAssignment[]; notify: (message: string, error?: boolean) => void }) {
  ...
  return <main className="workspace">
    ...
    {tab === 'inicio' && <SalesDashboard sales={allowedSales} markets={allowedMarkets} users={allowedUsers} onViewSales={() => setTab('ventas')} onExport={exportSales} />}
 ...
 function AnalystApp({ user, markets, setMarkets, users, setUsers, clients, setClients, sales, setSales, attendance, setAttendance, inventory, movements, assignments, setAssignments, setInventory, setMovements, closures, setClosures, notify }: { user: AppUser; markets: Market[]; setMarkets: (value: Market[]) => void; users: AppUser[]; setUsers: (value: AppUser[]) => void; clients: Client[]; setClients: (value: Client[]) => void; sales: Sale[]; setSales: (value: Sale[]) => void; attendance: Attendance[]; setAttendance: (value: Attendance[]) => void; inventory: MarketInventory[]; movements: InventoryMovement[]; assignments: PromoterAssignment[]; setAssignments: (value: PromoterAssignment[]) => void; setInventory: (value: MarketInventory[]) => void; setMovements: (value: InventoryMovement[]) => void; closures: SessionClosure[]; setClosures: (value: SessionClosure[]) => void; notify: (message: string, error?: boolean) => void }) {
   const [tab, setTab] = useState('inicio'); const [query, setQuery] = useState(''); const [salesQuery, setSalesQuery] = useState(''); const [modal, setModal] = useState<'user' | 'client' | 'market' | 'canje' | null>(null); const [editingSale, setEditingSale] = useState<Sale | null>(null); const [syncing, setSyncing] = useState(false);
    ...
    const exportUsers = () => { downloadCsv(`usuarios-${today}.csv`, ['ID', 'DNI', 'Nombre', 'Rol', 'Mercado', 'Stock degustación', 'Stock AVENA', 'Stock SPAGHETTI', 'Stock BATEA', 'Stock MANDIL', 'Estado'], visibleUsers.map(user => { const stock = userRedemptionStock(user); return [user.id, user.dni, user.name, user.roleLabel || user.role, marketMap[user.marketId || '']?.name || '', userTastingStock(user), stock.AVENA, stock.SPAGHETTI, stock.BATEA, stock.MANDIL, user.status]; })); notify('Reporte de usuarios descargado'); };
      const exportAttendance = () => { downloadCsv(`marcaciones-${today}.csv`, ['Código', 'Tipo', 'Fecha', 'Promotor', 'Rol promotor', 'Tienda', 'Departamento', 'Foto', 'Estado'], orderedAttendance.map(item => { const promoter = users.find(user => user.id === item.promoterId); const market = marketMap[item.marketId]; return [item.id, item.type, formatDate(item.date), promoter?.name || 'PROMOTOR NO IDENTIFICADO', item.promoterRoleLabel || promoter?.roleLabel || item.promoterRole || promoter?.role || 'ROL NO IDENTIFICADO', clients.find(client => client.id === item.clientId)?.name, market?.department || 'DEPARTAMENTO NO IDENTIFICADO', item.photo, item.status]; })); notify('Marcaciones descargadas'); };
  ...
  const filteredAdminSales = sales.filter(sale => {
   ...
   const adminCanjes: AdminCanje[] = [
      ...movements.filter(movement => movement.kind === 'CANJE' || (movement.kind === 'AJUSTE_CANJES' && (movement.canjeProductId || (movement.promoterId && movement.itemId))) || (movement.kind === 'AJUSTE_DEGUSTACION' && movement.promoterId && movement.degustacionProductId)).map(movement => ({ ...movement, source: 'movement' as const, canjeId: movement.id })),
  ...
  return <main className="workspace">
    ...
    <nav className="tabs" aria-label="Módulos">{tabs.map(([value, label]) => <button key={value} className={`tab ${tab === value ? 'active' : ''}`} onClick={() => setTab(value)} data-testid={`tab-${value}`}>{label}</button>)}</nav>
      ...
      {tab === 'mercados' && <section className="panel"><div className="panel-header"><div><h2>Mercados</h2><p>Catálogo almacenado en DigitalOcean PostgreSQL.</p></div><div className="panel-actions">{GOOGLE_INTEGRATIONS_ENABLED && <Btn variant="outline" onClick={importMarkets} disabled={syncing} testId="button-refresh-markets"><RefreshCw /> Actualizar hoja</Btn>}<Btn onClick={() => setModal('market')} testId="button-new-market"><Plus /> Nuevo mercado</Btn></div></div><div className="panel-body"><div className="module-table-wrap"><div className="module-table"><div className="module-table-row module-table-header cols-8"><span>Mercado</span><span>Región</span><span>Departamento</span><span>Provincia</span><span>Distrito</span><span>Clientes</span><span>Promotores</span><span>Acciones</span></div>{markets.map(market => <div className="module-table-row cols-8" key={market.id}><span><strong>{market.name}</strong><small>{market.id}</small></span><span><strong>{market.region || '—'}</strong></span><span>{market.department}</span><span>{market.province}</span><span>{market.district}</span><span><strong>{marketAssignmentSummary[market.id]?.clients || 0}</strong></span><span><strong>{marketAssignmentSummary[market.id]?.promoters || 0}</strong></span><span className="module-table-actions"><Btn variant="danger" onClick={() => deleteMarket(market)} testId={`button-delete-market-${market.id}`}><Trash2 /> Eliminar</Btn></span></div>)}</div></div></div></section>}
        {tab === 'usuarios' && <section className="panel"><div className="panel-header"><div><h2>Usuarios</h2><p>Administra usuarios en DigitalOcean o importa un CSV.</p></div><div className="panel-actions">{GOOGLE_INTEGRATIONS_ENABLED && <Btn variant="outline" onClick={importUsersFromSheet} disabled={syncing} testId="button-refresh-users"><RefreshCw /> Actualizar hoja</Btn>}<CsvImportButton label="Importar usuarios" onImport={importUsers} testId="button-import-users" /><CsvExampleButton onDownload={downloadUsersExample} testId="button-example-users" /><Btn onClick={() => setModal('user')} testId="button-new-user"><Plus /> Nuevo usuario</Btn></div></div><div className="panel-body"><div className="import-hint">DigitalOcean PostgreSQL conserva identidad, acceso, rol y estado. El stock se carga exclusivamente desde Canjes.</div><div className="module-table-wrap"><div className="module-table"><div className="module-table-row module-table-header cols-6"><span>Usuario</span><span>DNI</span><span>Rol</span><span>Asignación</span><span>Stock personal</span><span>Estado</span></div>{visibleUsers.map(user => <article className="module-table-row cols-6" key={user.id}><span><strong>{user.name}</strong><small>{user.id}</small></span><span><strong>{user.dni}</strong></span><span><strong>{user.roleLabel || user.role}</strong></span><span><strong>{user.marketId ? marketMap[user.marketId]?.name || 'Mercado asignado' : user.clientId ? 'Cliente vinculado' : 'Sin asignación'}</strong></span><span><strong>{isPromoterRole(user.role) ? `${userTastingStock(user)} degustación` : 'No aplica'}</strong><small>{isPromoterRole(user.role) ? redemptionStockText(userRedemptionStock(user)) : '—'}</small></span><span><StatusPill status={user.status} /></span></article>)}</div></div></div></section>}
      {tab === 'clientes' && <section className="panel"><div className="panel-header"><div><h2>Clientes</h2><p>Administra los clientes en DigitalOcean o importa un CSV.</p></div><div className="panel-actions">{GOOGLE_INTEGRATIONS_ENABLED && <Btn variant="outline" onClick={importClientsFromSheet} disabled={syncing} testId="button-refresh-clients"><RefreshCw /> Actualizar hoja</Btn>}<CsvImportButton label="Importar clientes" onImport={importClients} testId="button-import-clients" /><CsvExampleButton onDownload={downloadClientsExample} testId="button-example-clients" /><Btn onClick={() => setModal('client')} testId="button-new-client"><Plus /> Nuevo cliente</Btn></div></div><div className="panel-body sales-panel-body"><div className="sales-console-search"><Search /><Input value={query} onChange={setQuery} placeholder="Buscar cliente, código o mercado" testId="input-search-clients" /></div>{filteredClients.length ? <div className="module-table-wrap"><div className="module-table"><div className="module-table-row module-table-header cols-7"><span>Código</span><span>Cliente</span><span>Categoría</span><span>Mercado</span><span>Contacto</span><span>Estado</span><span>Acciones</span></div>{filteredClients.map(client => <article className="module-table-row cols-7" key={client.id}><span><b className="module-table-id">{client.code}</b><small>{client.id}</small></span><span><strong>{client.name}</strong></span><span><strong>{client.category || 'SIN CATEGORÍA'}</strong></span><span><strong>{marketMap[client.marketId]?.name || 'No identificado'}</strong><small>{marketMap[client.marketId]?.district || '—'}</small></span><span><strong>{client.phone || 'No registrado'}</strong></span><span><StatusPill status={client.status} /></span><span className="module-table-actions"><Btn variant="danger" onClick={() => deleteClient(client)} testId={`button-delete-client-${client.id}`}><Trash2 /> Eliminar</Btn></span></article>)}</div></div> : <Empty title="No hay coincidencias" detail="Prueba con otro nombre, código o mercado." />}</div></section>}
      ...
      {tab === 'degustacion' && <AdminDegustacionesModule consumos={tastingConsumptions} users={users} marketMap={marketMap} onDeleteConsumo={deleteTastingConsumption} />}
      {tab === 'asignaciones' && <AssignmentModule markets={markets} users={users} clients={clients} assignments={assignments} setAssignments={setAssignments} setUsers={setUsers} notify={notify} />}
       ...
       {tab === 'marcaciones' && <><section className="panel"><div className="panel-header"><div><h2>Marcaciones de asistencia</h2><p>Ordenadas desde la más reciente hasta la más antigua.</p></div><Btn variant="outline" onClick={exportAttendance}><Download /> Descargar</Btn></div><div className="panel-body">{orderedAttendance.length ? <div className="module-table-wrap"><div className="module-table"><div className="module-table-row module-table-header cols-7"><span>Código</span><span>Tienda</span><span>Promotor</span><span>Evento</span><span>Ubicación</span><span>Evidencia y fecha</span><span>Estado</span></div>{orderedAttendance.map(item => { const promoter = users.find(user => user.id === item.promoterId); const market = marketMap[item.marketId]; return <article className="module-table-row cols-7" key={item.id}><span><b className="module-table-id">{item.id}</b></span><span><strong>{clients.find(client => client.id === item.clientId)?.name || 'Tienda'}</strong><small>{market?.name || 'Mercado no identificado'}</small></span><span><strong>{promoter?.name || 'Promotor no identificado'}</strong><small>DNI {promoter?.dni || '—'} · {item.promoterRole || promoter?.role || 'Rol no identificado'}</small></span><span><b className={`sales-flag ${item.type === 'ENTRADA' ? 'yes' : 'no'}`}>{item.type}</b></span><span><strong>{market?.department || 'No identificado'}</strong><small>{market?.district || '—'}</small></span><span className="sales-evidence"><PhotoThumbnail label={item.type === 'ENTRADA' ? 'Entrada' : 'Salida'} src={item.photo} /><small>{formatDate(item.date)}</small></span><span><StatusPill status={item.status} /></span></article>; })}</div></div> : <Empty title="Aún no hay marcaciones" detail="Las entradas y salidas aparecerán aquí." />}</div></section>{automaticClosures.length > 0 && <section className="panel"><div className="panel-header"><div><h2>Evidencia de cierres automáticos</h2><p>Generados a las 11:59 p. m. cuando faltó el cierre manual.</p></div></div><div className="panel-body"><div className="module-table-wrap"><div className="module-table"><div className="module-table-row module-table-header cols-4"><span>Tienda</span><span>Promotor</span><span>Mercado y fecha</span><span>Estado</span></div>{automaticClosures.map(closure => { const promoter = users.find(current => current.id === closure.promoterId); const market = marketMap[closure.marketId]; const client = clients.find(current => current.id === closure.clientId); return <article className="module-table-row cols-4" key={closure.id}><span><strong>{client?.name || 'Cliente no identificado'}</strong><small>{closure.id}</small></span><span><strong>{promoter?.name || 'Promotor no identificado'}</strong></span><span><strong>{market?.name || 'Mercado no identificado'}</strong><small>{formatDate(closure.date)} · {closure.evidence || 'Cierre automático'}</small></span><span><span className="status pending">AUTOMÁTICO</span></span></article>; })}</div></div></div></section>}</>}
