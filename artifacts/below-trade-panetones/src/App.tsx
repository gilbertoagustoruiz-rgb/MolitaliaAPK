...
type AppUser = { id: string; dni: string; name: string; role: Role; roleLabel?: string; marketId?: string; clientId?: string; password?: string; status: Status; tastingStock?: number; redemptionStock?: RedemptionStock };
...
function NewUserModal({ markets, clients, onSave, close }: { markets: { value: string; label: string }[]; clients: Client[]; onSave: (user: AppUser) => void; close: () => void }) {
  ...
  const [tastingStock, setTastingStock] = useState('0'); const [redemptionStock, setRedemptionStock] = useState<RedemptionStock>(emptyRedemptionStock());
  const save = () => {
    if (dni.length !== 8 || !name.trim() || password.length < 8 || (isZoneManagerRole(role) && !marketId) || (role === 'CLIENTE' && !clientId)) return;
    const normalizedTasting = Math.max(0, Math.floor(Number(tastingStock) || 0));
    const normalizedRedemption = redemptionItems.reduce((result, item) => ({ ...result, [item.id]: Math.max(0, Math.floor(Number(redemptionStock[item.id]) || 0)) }), {} as RedemptionStock);
    onSave({ id: `USR-${Date.now()}`, dni, name: name.trim(), role, roleLabel: role, marketId: isZoneManagerRole(role) ? marketId : undefined, clientId: role === 'CLIENTE' ? clientId : undefined, password, status: 'ACTIVO', ...(isPromoterRole(role) ? { tastingStock: normalizedTasting, redemptionStock: normalizedRedemption } : {}) }); close();
  };
  return <Modal title="Crear usuario" detail="Define acceso, rol, mercado o cliente y, para promotores, su stock personal." close={close}><div className="form-grid"><Field label="DNI *"><Input value={dni} onChange={value => setDni(value.replace(/\D/g, ''))} maxLength={8} testId="input-new-user-dni" /></Field><Field label="Nombre completo *"><Input value={name} onChange={setName} testId="input-new-user-name" /></Field><SelectField label="Rol *" value={role} onChange={value => setRole(value as Role)} items={['PROMOTOR', ...selectablePromoterRoles, 'COORDINADOR', 'SUPERVISOR', 'ANALISTA', 'TRADE', 'ADMIN', 'CLIENTE'].map(value => ({ value, label: value }))} />{isZoneManagerRole(role) && <SelectField label="Mercado asignado *" value={marketId} onChange={setMarketId} items={markets} />}{role === 'CLIENTE' && <SelectField label="Cliente vinculado *" value={clientId} onChange={setClientId} items={clients.filter(client => client.status === 'ACTIVO').map(client => ({ value: client.id, label: `${client.code} · ${client.name}` }))} placeholder="Seleccionar cliente" />}{isPromoterRole(role) && <div className="form-grid full-field"><Field label="Stock degustación"><Input type="number" value={tastingStock} onChange={value => setTastingStock(value.replace(/[^\d]/g, ''))} min={0} step={1} testId="input-new-user-stock-degustacion" /></Field>{redemptionItems.map(item => <Field label={`Stock ${item.label}`} key={item.id}><Input type="number" value={redemptionStock[item.id]} onChange={value => setRedemptionStock({ ...redemptionStock, [item.id]: value === '' ? 0 : Math.max(0, Math.floor(Number(value))) })} min={0} step={1} testId={`input-new-user-stock-${item.id.toLowerCase()}`} /></Field>)}</div>}<Field label="Clave temporal *"><Input value={password} onChange={value => setPassword(value)} type="password" testId="input-new-user-password" /></Field></div><div className="modal-actions"><Btn variant="outline" onClick={close}>Cancelar</Btn><Btn onClick={save} testId="button-create-user">Crear usuario</Btn></div></Modal>;
}
...
function CoordinatorApp({ user, markets, users, clients, sales, inventory, movements, assignments, notify }: { user: AppUser; markets: Market[]; users: AppUser[]; clients: Client[]; sales: Sale[]; inventory: MarketInventory[]; movements: InventoryMovement[]; assignments: PromoterAssignment[]; notify: (message: string, error?: boolean) => void }) {
 ...
 function AnalystApp({ user, markets, setMarkets, users, setUsers, clients, setClients, sales, setSales, attendance, setAttendance, inventory, movements, assignments, setAssignments, setInventory, setMovements, closures, setClosures, notify }: { user: AppUser; markets: Market[]; setMarkets: (value: Market[]) => void; users: AppUser[]; setUsers: (value: AppUser[]) => void; clients: Client[]; setClients: (value: Client[]) => void; sales: Sale[]; setSales: (value: Sale[]) => void; attendance: Attendance[]; setAttendance: (value: Attendance[]) => void; inventory: MarketInventory[]; movements: InventoryMovement[]; assignments: PromoterAssignment[]; setAssignments: (value: PromoterAssignment[]) => void; setInventory: (value: MarketInventory[]) => void; setMovements: (value: InventoryMovement[]) => void; closures: SessionClosure[]; setClosures: (value: SessionClosure[]) => void; notify: (message: string, error?: boolean) => void }) {
   ...
   const activeMarkets = markets.filter(market => market.status === 'ACTIVO'); const marketMap = Object.fromEntries(markets.map(market => [market.id, market])); const marketAssignmentSummary = useMemo(() => Object.fromEntries(markets.map(market => [market.id, { clients: clients.filter(client => client.marketId === market.id && client.status === 'ACTIVO').length, promoters: users.filter(current => { if (!isPromoterRole(current.role) || current.status !== 'ACTIVO') return false; const assignment = assignments.find(item => assignmentMatchesUser(item, current)); return assignment ? assignment.marketIds.includes(market.id) : current.marketId === market.id; }).length }])), [markets, clients, users, assignments]); const today = new Date().toISOString().slice(0, 10);
    ...
    const exportUsers = () => { downloadCsv(`usuarios-${today}.csv`, ['ID', 'DNI', 'Nombre', 'Rol', 'Mercado', 'Stock degustación', 'Stock AVENA', 'Stock SPAGHETTI', 'Stock BATEA', 'Stock MANDIL', 'Estado'], users.map(user => { const stock = userRedemptionStock(user); return [user.id, user.dni, user.name, user.roleLabel || user.role, marketMap[user.marketId || '']?.name || '', userTastingStock(user), stock.AVENA, stock.SPAGHETTI, stock.BATEA, stock.MANDIL, user.status]; })); notify('Reporte de usuarios descargado'); };
  ...
  const downloadUsersExample = () => {
    const market = activeMarkets[0] || markets[0];
     downloadCsv('ejemplo-usuarios.csv', ['DNI', 'Nombre', 'Rol', 'Mercado', 'Stock degustación', 'Stock AVENA', 'Stock SPAGHETTI', 'Stock BATEA', 'Stock MANDIL', 'Estado'], [['87654322', 'Promotor Ejemplo', 'PROMOTOR', market?.name || 'MERCADO MODELO', 20, 10, 10, 2, 2, 'ACTIVO']]);
