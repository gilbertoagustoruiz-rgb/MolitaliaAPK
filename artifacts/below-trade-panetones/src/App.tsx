...
type Sale = { id: string; promoterId: string; clientId: string; marketId: string; mode: 'UNIDADES' | 'PLANCHAS'; units: number; planchas?: number; mix: Record<string, number>; bonus?: string; receiptPhoto: string; exchangePhoto?: string; date: string; status: 'SINCRONIZADA' | 'PENDIENTE' };
type Attendance = { id: string; promoterId: string; clientId: string; marketId: string; type: 'ENTRADA' | 'SALIDA'; photo: string; date: string; status: 'SINCRONIZADA' | 'PENDIENTE' };
...
function Input({ value, onChange, placeholder, type = 'text', min, max, maxLength, autoComplete, testId }: { value: string | number; onChange: (value: string) => void; placeholder?: string; type?: string; min?: number; max?: number; maxLength?: number; autoComplete?: string; testId?: string }) {
  return <input className="input" value={value} onChange={event => onChange(event.target.value)} placeholder={placeholder} type={type} min={min} max={max} maxLength={maxLength} autoComplete={autoComplete} data-testid={testId} />;
}
...
function AnalystApp({ markets, setMarkets, users, setUsers, clients, setClients, sales, attendance, notify }: { markets: Market[]; setMarkets: (value: Market[]) => void; users: AppUser[]; setUsers: (value: AppUser[]) => void; clients: Client[]; setClients: (value: Client[]) => void; sales: Sale[]; attendance: Attendance[]; notify: (message: string, error?: boolean) => void }) {
  ...
  const exportSales = () => { downloadCsv(`ventas-${today}.csv`, ['Código venta', 'Fecha', 'Promotor', 'Cliente', 'Mercado', 'Tipo', 'Unidades', 'Bonificación', 'Estado'], sales.map(sale => [sale.id, formatDate(sale.date), users.find(user => user.id === sale.promoterId)?.name, clients.find(client => client.id === sale.clientId)?.name, marketMap[sale.marketId]?.name, sale.mode, sale.units, sale.bonus || 'Sin canje', sale.status])); notify('Reporte de ventas descargado'); };
  ...
  const exportSummary = () => { const withBonus = sales.filter(sale => sale.bonus).length; downloadCsv(`resumen-${today}.csv`, ['Fecha', 'Mercados activos', 'Usuarios activos', 'Clientes activos', 'Ventas', 'Con canje', 'Unidades'], [[new Date().toLocaleString('es-PE'), activeMarkets.length, users.filter(user => user.status === 'ACTIVO').length, clients.filter(client => client.status === 'ACTIVO').length, sales.length, withBonus, sales.reduce((sum, sale) => sum + sale.units, 0)]]); notify('Resumen descargado'); };
  ...
  const tabs = [['inicio', 'Resumen'], ['mercados', 'Mercados'], ['usuarios', 'Usuarios'], ['clientes', 'Clientes'], ['ventas', 'Ventas'], ['marcaciones', 'Marcaciones']];
...
function PromoterApp({ user, market, clients, sales, setSales, attendance, setAttendance, notify }: { user: AppUser; market?: Market; clients: Client[]; sales: Sale[]; setSales: (value: Sale[]) => void; attendance: Attendance[]; setAttendance: (value: Attendance[]) => void; notify: (message: string, error?: boolean) => void }) {
  ...
  const totalMix = mix.TODINNO + mix.COSTA + mix.PASQUALINO; const total = mode === 'UNIDADES' ? unitQty : totalMix; const bonus = bonusFor(mode, total, planchas); const validMix = mode === 'UNIDADES' || (totalMix === planchas * 6 && Object.values(mix).every(value => value >= 1)); const canConfirm = Boolean(clientId && receipt && validMix && (!bonus || exchange) && !(mode === 'PLANCHAS' && planchas > 80));
  ...
  const saveSale = () => {
    if (!canConfirm) { notify(mode === 'PLANCHAS' && planchas > 80 ? 'Requiere autorización previa de Trade' : 'Completa venta y evidencias', true); return; }
    const id = `VTA-${new Date().getFullYear()}-${String(sales.length + 1).padStart(6, '0')}`; const sale: Sale = { id, promoterId: user.id, clientId, marketId: user.marketId || '', mode, units: total, planchas: mode === 'PLANCHAS' ? planchas : undefined, mix: mode === 'PLANCHAS' ? mix : { [products.find(product => product.sku === sku)?.brand || 'PRODUCTO']: unitQty }, bonus, receiptPhoto: `BOLETA - ${id}.jpg`, exchangePhoto: bonus ? `${bonus} - CLIENTE - ${id}.jpg` : undefined, date: new Date().toISOString(), status: navigator.onLine ? 'SINCRONIZADA' : 'PENDIENTE' };
    const next = [sale, ...sales]; setSales(next); writeStore('bt-sales', next); setReceipt(null); setExchange(null); setView('LISTA'); notify(bonus ? 'Venta y canje registrados' : 'Venta sin canje registrada');
  };
  ...
  return <main className="promoter-page"><div className="promoter-layout"><PromoterNav active={module} onChange={value => { setModule(value); setView('LISTA'); setSearch(''); }} /><div className="promoter-content">
    ...
    <div className="market-banner"><span className="market-pin"><MapPin /></span><div><small>MERCADO ASIGNADO</small><strong>{market.name}</strong><p>{market.district} · {market.province} · {market.department}</p></div><StatusPill status="ACTIVO" /></div>
...
export default function App() {
  ...
  return <>{activeUser ? <Shell user={activeUser} logout={logout}>{activeUser.role === 'PROMOTOR' ? <PromoterApp user={activeUser} market={markets.find(market => market.id === activeUser.marketId)} clients={clients} sales={sales} setSales={setSales} attendance={attendance} setAttendance={setAttendance} notify={notify} /> : <AnalystApp markets={markets} setMarkets={setMarkets} users={users} setUsers={setUsers} clients={clients} setClients={setClients} sales={sales} attendance={attendance} notify={notify} />}</Shell> : <Login onLogin={setUser} notify={notify} />}<ToastView toast={toast} clear={() => setToast(null)} /></>;
}