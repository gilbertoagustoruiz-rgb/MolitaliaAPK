...
function Shell({ user, logout, theme, toggleTheme, children }: { user: AppUser; logout: () => void; theme: 'light' | 'dark'; toggleTheme: () => void; children: ReactNode }) {
  ...
  return <div className="bt-app"><header className="topbar"><Logo compact /><div className="campaign"><small>Campaña</small><strong>Panetones Molitalia</strong></div><div className="top-user"><Btn variant="ghost" className="theme-toggle" onClick={toggleTheme} testId="button-toggle-theme">{theme === 'dark' ? <Sun /> : <Moon />}<span>{theme === 'dark' ? 'Día' : 'Noche'}</span></Btn><div className="top-user-info"><strong>{user.name}</strong><small>{user.roleLabel || user.role}</small></div><Btn variant="ghost" onClick={logout} testId="button-logout"><LogOut /> Salir</Btn></div></header>{!online && <div className="offline-bar" role="status" aria-live="polite" data-testid="status-offline"><WifiOff /> Sin conexión · cambios guardados en este dispositivo</div>}{children}</div>;
}
