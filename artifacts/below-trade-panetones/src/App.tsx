...
function AnalystApp({ user, markets, setMarkets, users, setUsers, clients, setClients, sales, attendance, inventory, movements, assignments, setAssignments, setInventory, setMovements, notify }: { user: AppUser; markets: Market[]; setMarkets: (value: Market[]) => void; users: AppUser[]; setUsers: (value: AppUser[]) => void; clients: Client[]; setClients: (value: Client[]) => void; sales: Sale[]; attendance: Attendance[]; inventory: MarketInventory[]; movements: InventoryMovement[]; assignments: PromoterAssignment[]; setAssignments: (value: PromoterAssignment[]) => void; setInventory: (value: MarketInventory[]) => void; setMovements: (value: InventoryMovement[]) => void; notify: (message: string, error?: boolean) => void }) {
  ...
  const exportSales = () => { downloadCsv(`ventas-${today}.csv`, ['Código venta', 'Fecha', 'Promotor', 'Cliente', 'Mercado', 'Tipo', 'Marca / producto', 'Unidades por marca', 'Monto unitario por marca (S/)', 'Unidades totales', 'Peso (kg)', 'Ingreso total (S/)', 'Bonificación', 'Comentario', 'Stock degustación entregado', 'Degustación utilizada', 'Stock degustación restante', 'Canjes entregados', 'Canjes realizados', 'Stock canjes restante', 'Detalle stock canjes restante', 'Estado'], sales.map(sale => { const breakdown = saleExportBreakdown(sale); const stock = summarizeMarketStock(sale.marketId, inventory, movements); return [sale.id, formatDate(sale.date), users.find(user => user.id === sale.promoterId)?.name, clients.find(client => client.id === sale.clientId)?.name, marketMap[sale.marketId]?.name, sale.mode, breakdown.map(item => item.label).join(' | '), breakdown.map(item => item.units).join(' | '), breakdown.map(item => item.unitPrice.toFixed(2)).join(' | '), sale.units, (sale.weightKg ?? 0).toFixed(3), (sale.amountSoles ?? 0).toFixed(2), sale.bonus || 'Sin canje', sale.comment || '', stock.tastingDelivered, stock.tastingUsed, stock.tastingRemaining, sumRedemptionStock(stock.redemptionDelivered), sumRedemptionStock(stock.redemptionUsed), sumRedemptionStock(stock.redemptionRemaining), redemptionStockText(stock.redemptionRemaining), sale.status]; })); notify('Reporte de ventas con stock por mercado descargado'); };
