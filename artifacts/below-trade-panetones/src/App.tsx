...
import { Camera, Check, CheckCircle2, Clock3, Download, FileSpreadsheet, Gift, LogOut, MapPin, PackageCheck, Plus, RefreshCw, Search, ShieldCheck, ShoppingBag, Smartphone, Store, Trash2, Upload, UserRound, Users, Wifi, WifiOff, X } from 'lucide-react';

type Role = 'ANALISTA' | 'PROMOTOR' | 'PROMOTOR ROTATIVO' | 'PROMOTOR PERMANENTE' | 'SUPERVISOR' | 'TRADE' | 'ADMIN' | 'CLIENTE';
...
function importedUserFromRecord(record: Record<string, string>, index: number, markets: Market[]): AppUser | null {
  ...
  const roleValue = rawRole === 'promotorrotativo' ? 'PROMOTOR ROTATIVO' : rawRole === 'promotorpermanente' ? 'PROMOTOR PERMANENTE' : rawRole === 'promotor' ? 'PROMOTOR' : rawRole === 'coordinador' ? 'SUPERVISOR' : rawRole === 'cliente' ? 'CLIENTE' : rawRole.toUpperCase() as Role;
  ...
  if (!/^\d{8}$/.test(dni) || !name || ![...promoterRoles, 'SUPERVISOR', 'ANALISTA', 'TRADE', 'ADMIN', 'CLIENTE'].includes(roleValue) || (isPromoterRole(roleValue) && marketValue && !marketId)) return null;
  return { id: csvField(record, ['id', 'codigo', 'idusuario', 'idpromotor']) || `USR-IMP-${index + 1}`, dni, name, role: roleValue, roleLabel: roleLabel || roleValue, marketId: isPromoterRole(roleValue) ? marketId : undefined, clientId: roleValue === 'CLIENTE' ? csvField(record, ['idcliente', 'clienteid', 'codigocliente']) || undefined : undefined, password: csvField(record, ['clave', 'password', 'contrasena']) || undefined, status: csvStatus(csvField(record, ['estado', 'status'])) };
}
...
function NewUserModal({ markets, clients, onSave, close }: { markets: { value: string; label: string }[]; clients: Client[]; onSave: (user: AppUser) => void; close: () => void }) {
  ...
  const save = () => {
    if (dni.length !== 8 || !name.trim() || password.length < 8 || (isPromoterRole(role) && !marketId) || (role === 'CLIENTE' && !clientId)) return;
    onSave({ id: `USR-${Date.now()}`, dni, name: name.trim(), role, roleLabel: role, marketId: isPromoterRole(role) ? marketId : undefined, clientId: role === 'CLIENTE' ? clientId : undefined, password, status: 'ACTIVO' }); close();
  };
  return <Modal title="Crear usuario" detail="Define acceso, rol y mercado o cliente." close={close}><div className="form-grid"><Field label="DNI *"><Input value={dni} onChange={value => setDni(value.replace(/\D/g, ''))} maxLength={8} testId="input-new-user-dni" /></Field><Field label="Nombre completo *"><Input value={name} onChange={setName} testId="input-new-user-name" /></Field><SelectField label="Rol *" value={role} onChange={value => setRole(value as Role)} items={[...selectablePromoterRoles, 'SUPERVISOR', 'ANALISTA', 'TRADE', 'ADMIN', 'CLIENTE'].map(value => ({ value, label: value }))} />{isPromoterRole(role) && <SelectField label="Mercado asignado *" value={marketId} onChange={setMarketId} items={markets} />}{role === 'CLIENTE' && <SelectField label="Cliente vinculado *" value={clientId} onChange={setClientId} items={clients.filter(client => client.status === 'ACTIVO').map(client => ({ value: client.id, label: `${client.code} · ${client.name}` }))} placeholder="Seleccionar cliente" />}<Field label="Clave temporal *"><Input value={password} onChange={value => setPassword(value)} type="password" testId="input-new-user-password" /></Field></div><div className="modal-actions"><Btn variant="outline" onClick={close}>Cancelar</Btn><Btn onClick={save} testId="button-create-user">Crear usuario</Btn></div></Modal>;
}
...
function AdminCanjesModule({ canjes, marketMap, onCreate, onDelete, notify }: { canjes: AdminCanje[]; marketMap: Record<string, Market>; onCreate: () => void; onDelete: (canje: AdminCanje) => void; notify: (message: string, error?: boolean) => void }) {
  const exportCanjes = () => {
...
function AdminOnlyApp({ notify }: { notify: (message: string, error?: boolean) => void }) {
  const [cleaning, setCleaning] = useState(false);
  const cleanupCatalogs = async () => {
    if (!window.confirm('Se eliminarán definitivamente todos los mercados, clientes, ventas, marcaciones, inventario, canjes, asignaciones, cierres y usuarios que no sean ANALISTA. Los precios y el usuario ANALISTA se conservarán. ¿Empezar desde cero?')) return;
    setCleaning(true);
    try {
      const response = await fetch(`${APP_STORAGE_ADMIN}/cleanup`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ confirmation: 'LIMPIAR_MERCADOS_CLIENTES_CANJES' }) });
      const payload = await response.json() as { message?: string };
      if (!response.ok) throw new Error(payload.message || 'No se pudo limpiar la información');
      localStorage.removeItem(DEFAULT_STOCK_SEED_KEY);
      notify('Marcaciones, inventario y demás datos operativos eliminados.');
      window.location.reload();
    } catch (error) {
      notify(error instanceof Error ? error.message : 'No se pudo limpiar la información', true);
      setCleaning(false);
    }
  };
  return <main className="workspace admin-only-workspace">
    <div className="page-head"><div><span className="eyebrow">ADMINISTRACIÓN</span><h1>Control inicial</h1><p>Esta cuenta solo puede reiniciar la información operativa de la campaña.</p></div></div>
    <section className="admin-warning admin-only-warning"><Trash2 /><span><strong>Limpieza inicial</strong><small>Elimina mercados, clientes, ventas, marcaciones, inventario, canjes, asignaciones, cierres y usuarios. Los precios y el usuario ANALISTA se conservan.</small></span><Btn variant="danger" onClick={cleanupCatalogs} disabled={cleaning} testId="button-cleanup-catalogs">{cleaning ? 'Procesando…' : 'Empezar desde cero'}</Btn></section>
  </main>;
}
...
function AssignmentModule({ markets, users, clients, assignments, setAssignments, setUsers, notify }: { markets: Market[]; users: AppUser[]; clients: Client[]; assignments: PromoterAssignment[]; setAssignments: (value: PromoterAssignment[]) => void; setUsers: (value: AppUser[]) => void; notify: (message: string, error?: boolean) => void }) {
  const promoters = users.filter(user => isPromoterRole(user.role) && user.status === 'ACTIVO');
...
function SalesDashboard({ sales, markets, users, inventory, movements, onViewSales, onExport }: { sales: Sale[]; markets: Market[]; users: AppUser[]; inventory: MarketInventory[]; movements: InventoryMovement[]; onViewSales: () => void; onExport: () => void }) {
  ...
  const stockRows = useMemo(() => markets.map(market => summarizeMarketStock(market.id, inventory, movements)), [markets, inventory, movements]);
  ...
  return <div className="sales-dashboard">
    ...
    <div className="summary-sections">
     ...
     <section className="panel"><div className="panel-header"><div><h2>Stock por mercado</h2><p>Control de lo entregado, utilizado y el saldo que queda por degustación y canjes.</p></div></div><div className="panel-body summary-table-wrap">{stockRows.length ? <div className="stock-reconciliation-table"><div className="stock-reconciliation-row header"><span>Mercado</span><span>Degustación entregada</span><span>Utilizada</span><span>Saldo degustación</span><span>Canjes entregados</span><span>Canjes usados</span><span>Saldo canjes</span></div>{stockRows.map(row => <div className="stock-reconciliation-row" key={row.marketId}><div><strong>{markets.find(market => market.id === row.marketId)?.name || 'Mercado no identificado'}</strong><small>{markets.find(market => market.id === row.marketId)?.district || '—'}</small></div><span>{row.tastingDelivered}<small>panetones</small></span><span>{row.tastingUsed}<small>panetones</small></span><b>{row.tastingRemaining}<small>panetones</small></b><span>{sumRedemptionStock(row.redemptionDelivered)}<small>{redemptionStockText(row.redemptionDelivered)}</small></span><span>{sumRedemptionStock(row.redemptionUsed)}<small>{redemptionStockText(row.redemptionUsed)}</small></span><b>{sumRedemptionStock(row.redemptionRemaining)}<small>{redemptionStockText(row.redemptionRemaining)}</small></b></div>)}</div> : <Empty title="Sin mercados registrados" detail="Los mercados aparecerán aquí al cargar el catálogo." />}</div></section>
    <div className="summary-sections">
 ...
 function AnalystApp({ user, markets, setMarkets, users, setUsers, clients, setClients, sales, setSales, attendance, setAttendance, inventory, movements, assignments, setAssignments, setInventory, setMovements, closures, setClosures, notify }: { user: AppUser; markets: Market[]; setMarkets: (value: Market[]) => void; users: AppUser[]; setUsers: (value: AppUser[]) => void; clients: Client[]; setClients: (value: Client[]) => void; sales: Sale[]; setSales: (value: Sale[]) => void; attendance: Attendance[]; setAttendance: (value: Attendance[]) => void; inventory: MarketInventory[]; movements: InventoryMovement[]; assignments: PromoterAssignment[]; setAssignments: (value: PromoterAssignment[]) => void; setInventory: (value: MarketInventory[]) => void; setMovements: (value: InventoryMovement[]) => void; closures: SessionClosure[]; setClosures: (value: SessionClosure[]) => void; notify: (message: string, error?: boolean) => void }) {
  const [tab, setTab] = useState('inicio'); const [query, setQuery] = useState(''); const [modal, setModal] = useState<'user' | 'client' | 'market' | 'canje' | 'degustacion' | null>(null); const [syncing, setSyncing] = useState(false);
   const activeMarkets = markets.filter(market => market.status === 'ACTIVO'); const marketMap = Object.fromEntries(markets.map(market => [market.id, market])); const marketAssignmentSummary = useMemo(() => Object.fromEntries(markets.map(market => [market.id, { clients: clients.filter(client => client.marketId === market.id && client.status === 'ACTIVO').length, promoters: users.filter(current => { if (!isPromoterRole(current.role) || current.status !== 'ACTIVO') return false; const assignment = assignments.find(item => assignmentMatchesUser(item, current)); return assignment ? assignment.marketIds.includes(market.id) : current.marketId === market.id; }).length }])), [markets, clients, users, assignments]); const today = new Date().toISOString().slice(0, 10);
  ...
  const mergeImportedUsers = async (records: Record<string, string>[]) => {
    ...
    records.forEach((record, index) => {
       const dni = csvField(record, ['dni', 'documento', 'documentoidentidad']); const firstName = csvField(record, ['nombre', 'nombrecompleto', 'usuario', 'nombres']); const lastName = csvField(record, ['apellido', 'apellidos']); const name = [firstName, lastName].filter(Boolean).join(' '); const roleLabel = csvField(record, ['rol', 'cargo', 'perfil', 'tipousuario']).trim().toUpperCase(); const rawRole = normalizeCsvHeader(roleLabel); const roleValue = rawRole === 'promotorrotativo' ? 'PROMOTOR ROTATIVO' : rawRole === 'promotorpermanente' ? 'PROMOTOR PERMANENTE' : rawRole === 'promotor' ? 'PROMOTOR' : rawRole === 'coordinador' ? 'SUPERVISOR' : rawRole.toUpperCase() as Role; const marketValue = csvField(record, ['marketid', 'idmercado', 'idmerc', 'mercadoid', 'mercado', 'market', 'nombremercado']);
      const marketId = marketValue ? csvMarketId(marketValue, markets) : undefined;
       if (!/^\d{8}$/.test(dni) || !name || ![...promoterRoles, 'SUPERVISOR', 'ANALISTA', 'TRADE', 'ADMIN', 'CLIENTE'].includes(roleValue) || (isPromoterRole(roleValue) && marketValue && !marketId)) { skipped += 1; return; }
        imported.push({ id: csvField(record, ['id', 'codigo', 'idusuario', 'idpromotor']) || `USR-IMP-${importId}-${index + 1}`, dni, name, role: roleValue, roleLabel: roleLabel || roleValue, marketId: isPromoterRole(roleValue) ? marketId : undefined, clientId: roleValue === 'CLIENTE' ? csvField(record, ['idcliente', 'clienteid', 'codigocliente']) || undefined : undefined, password: csvField(record, ['clave', 'password', 'contrasena']) || undefined, status: csvStatus(csvField(record, ['estado', 'status'])) });
    });
    if (!imported.length) throw new Error('No se encontraron filas válidas. Revisa DNI, nombre, rol y mercado para promotores.');
     const marketLoads = new Map(markets.map(market => [market.id, users.filter(item => isPromoterRole(item.role) && item.status === 'ACTIVO' && item.marketId === market.id).length]));
     imported.forEach(item => { if (!isPromoterRole(item.role) || item.marketId) return; const target = markets.slice().sort((first, second) => (marketLoads.get(first.id) || 0) - (marketLoads.get(second.id) || 0))[0]; if (target) { item.marketId = target.id; marketLoads.set(target.id, (marketLoads.get(target.id) || 0) + 1); } });
  ...
  const downloadInventoryExample = () => {
   ...
   const saveCanje = async (canje: InventoryMovement) => { try { await adminRequest('/canjes', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ canje }) }); notify('Canje registrado e inventario actualizado'); } catch (error) { notify(error instanceof Error ? error.message : 'No se pudo crear el canje', true); } };
  ...
  const filteredClients = clients.filter(client => `${client.name} ${client.code} ${marketMap[client.marketId]?.name || ''}`.toLowerCase().includes(query.toLowerCase()));
   ...
   const adminDegustaciones: AdminDegustacion[] = movements.filter(movement => movement.kind === 'AJUSTE_DEGUSTACION' && movement.degustacionProductId).map(movement => ({ ...movement, source: 'movement' as const, degustacionId: movement.id }));
     if (user.role === 'ADMIN') return <AdminOnlyApp notify={notify} />;
  ...
  return <main className="workspace">
    ...
    <nav className="tabs" aria-label="Módulos">{tabs.map(([value, label]) => <button key={value} className={`tab ${tab === value ? 'active' : ''}`} onClick={() => setTab(value)} data-testid={`tab-${value}`}>{label}</button>)}</nav>
      ...
      {tab === 'clientes' && <section className="panel"><div className="panel-header"><div><h2>Clientes</h2><p>Importa, ingresa y elimina clientes manualmente.</p></div><div className="panel-actions"><Btn variant="outline" onClick={importClientsFromSheet} disabled={syncing} testId="button-refresh-clients"><RefreshCw /> Actualizar hoja</Btn><CsvImportButton label="Importar clientes" onImport={importClients} testId="button-import-clients" /><CsvExampleButton onDownload={downloadClientsExample} testId="button-example-clients" /><Btn onClick={() => setModal('client')} testId="button-new-client"><Plus /> Nuevo cliente</Btn></div></div><div className="panel-body"><div className="import-hint">Origen conectado: Google Sheets · También puedes crear y eliminar desde esta pantalla.</div><div className="search-row"><div className="search-wrap"><Search /><Input value={query} onChange={setQuery} placeholder="Buscar cliente, código o mercado" testId="input-search-clients" /></div></div><div className="record-list">{filteredClients.length ? filteredClients.map(client => <article className="record" key={client.id}><span className="record-icon"><Store /></span><div className="record-main"><strong>{client.name}</strong><small>{client.code}{client.phone ? ` · ${client.phone}` : ''} · Categoría: {client.category || 'SIN CATEGORÍA'}</small><em><MapPin /> {marketMap[client.marketId]?.name}</em></div><StatusPill status={client.status} /><Btn variant="danger" onClick={() => deleteClient(client)} testId={`button-delete-client-${client.id}`}><Trash2 /></Btn></article>) : <Empty title="No hay coincidencias" detail="Prueba con otro nombre, código o mercado." />}</div></div></section>}
       {tab === 'canjes' && <AdminCanjesModule canjes={adminCanjes} marketMap={marketMap} onCreate={() => setModal('canje')} onDelete={deleteCanje} notify={notify} />}
      ...
      {tab === 'ventas' && <section className="panel"><div className="panel-header"><div><h2>Ventas y evidencias</h2><p>Seguimiento de registros por promotor, ubicación y saldo actualizado del mercado.</p></div><Btn variant="outline" onClick={exportSales}><Download /> Descargar</Btn></div><div className="panel-body">{sales.length ? <div className="record-list">{sales.map(sale => { const promoter = users.find(item => item.id === sale.promoterId); const stock = summarizeMarketStock(sale.marketId, inventory, movements); return <article className="record" key={sale.id}><span className="record-icon"><ShoppingBag /></span><div className="record-main"><strong>{clients.find(client => client.id === sale.clientId)?.name || 'Tienda'}</strong><small>Promotor: {promoter?.name || 'No identificado'} · DNI {promoter?.dni || '—'} · Rol: {sale.promoterRoleLabel || promoter?.roleLabel || sale.promoterRole || promoter?.role || 'Rol no identificado'} · {marketMap[sale.marketId]?.name || 'Mercado no identificado'}</small><small><MapPin /> {saleMarketLocationText(sale, markets)}</small><small>{sale.id} · {sale.units} unidades · {sale.mode} · {formatSoles(sale.amountSoles)}</small><em><Gift /> {sale.bonus ? `${sale.redemptionCount ?? 1} canje${(sale.redemptionCount ?? 1) === 1 ? '' : 's'} · ${sale.bonus}` : 'Sin canje'} · {formatDate(sale.date)}</em><em><PackageCheck /> Saldo del mercado: {stock.tastingRemaining} degustación · {sumRedemptionStock(stock.redemptionRemaining)} canjes</em>{sale.comment && <em>Comentario: {sale.comment}</em>}</div><StatusPill status={sale.status} /></article>; })}</div> : <Empty />}</div></section>}
      {tab === 'marcaciones' && <section className="panel"><div className="panel-header"><div><h2>Marcaciones de asistencia</h2><p>Entradas y salidas registradas por los promotores.</p></div><Btn variant="outline" onClick={exportAttendance}><Download /> Descargar</Btn></div><div className="panel-body">{attendance.length ? <div className="record-list">{attendance.map(item => { const promoter = users.find(user => user.id === item.promoterId); return <article className="record" key={item.id}><span className="record-icon"><Clock3 /></span><div className="record-main"><strong>{clients.find(client => client.id === item.clientId)?.name || 'Tienda'}</strong><small>{promoter?.name || 'Promotor no identificado'} · Rol: {item.promoterRole || promoter?.role || 'Rol no identificado'} · {formatDate(item.date)}</small><em><Camera /> {item.photo}</em></div><span className={`status ${item.type === 'ENTRADA' ? 'active' : 'pending'}`}>{item.type}</span></article>; })}</div> : <Empty title="Aún no hay marcaciones" detail="Las entradas y salidas aparecerán aquí." />}</div></section>}
