...
function AdminCanjesModule({ canjes, marketMap, users, onCreate, onDelete, onCleanup, canCleanup, notify }: { canjes: AdminCanje[]; marketMap: Record<string, Market>; users: AppUser[]; onCreate: () => void; onDelete: (canje: AdminCanje) => void; onCleanup: () => void; canCleanup: boolean; notify: (message: string, error?: boolean) => void }) {
  ...
  return <section className="admin-module">
    ...
    {canCleanup && <div className="admin-warning"><Trash2 /><span><strong>Limpieza inicial</strong><small>Elimina todos los datos operativos y todos los usuarios excepto ANALISTA. Los precios se conservan.</small></span><Btn variant="danger" onClick={onCleanup} testId="button-cleanup-catalogs">Empezar desde cero</Btn></div>}
     <section className="panel"><div className="panel-header"><div><h2>{canjes.length} movimientos registrados</h2><p>Eliminar un abastecimiento revierte el stock cargado.</p></div></div><div className="panel-body">{canjes.length ? <div className="record-list">{canjes.map(canje => { const promoter = canje.promoterId ? users.find(current => current.id === canje.promoterId) : undefined; return <article className="record" key={`${canje.source}-${canje.canjeId}`}><span className="record-icon"><Gift /></span><div className="record-main"><strong>{canjeProductLabel(canje.canjeProductId, canje.canjeProductLabel || canje.degustacionProductLabel || canje.sale?.bonus || (canje.itemId ? redemptionLabel(canje.itemId) : undefined))}</strong><small>{promoter ? `Stock de ${promoter.name}` : marketMap[canje.marketId]?.name || 'Mercado no identificado'} · {canje.quantity} unidad{canje.quantity === 1 ? '' : 'es'}</small><em>{canje.source === 'sale' ? 'Venta' : canje.kind === 'AJUSTE_DEGUSTACION' ? 'Abastecimiento de degustación' : 'Abastecimiento'} · {canje.actorName} · {formatDate(canje.date)}</em></div><Btn variant="danger" onClick={() => onDelete(canje)} testId={`button-delete-canje-${canje.canjeId}`}><Trash2 /> Eliminar</Btn></article>; })}</div> : <Empty title="Aún no hay movimientos" detail="Abastece el stock personal de un promotor." />}</div></section>
  </section>;
}
...
function AdminDegustacionesModule({ consumos, users, marketMap, onDeleteConsumo }: { consumos: InventoryMovement[]; users: AppUser[]; marketMap: Record<string, Market>; onDeleteConsumo: (consumo: InventoryMovement) => void }) {
  return <section className="admin-module">
    ...
    <section className="panel"><div className="panel-header"><div><h2>{consumos.length} consumos registrados</h2><p>Ordenados desde el más reciente hasta el más antiguo.</p></div></div><div className="panel-body">{consumos.length ? <div className="record-list sales-record-list">{consumos.map(consumo => {
      const promoter = users.find(current => current.id === (consumo.promoterId || consumo.actorId));
      const market = marketMap[consumo.marketId];
      return <article className="record sale-admin-record tasting-admin-record" key={consumo.id}>
        <span className="record-icon"><PackageCheck /></span>
        <div className="record-main sale-card-main">
          <div className="sale-card-heading"><div><strong>{promoter?.name || consumo.actorName || 'Promotor no identificado'}</strong><small>DNI {promoter?.dni || '—'} · {promoter?.roleLabel || promoter?.role || 'Promotor'}</small></div><b>{consumo.quantity} unidad{consumo.quantity === 1 ? '' : 'es'}</b></div>
          <div className="sale-card-meta"><span><Clock3 /> {formatDate(consumo.date)}</span><span><MapPin /> {market?.name || 'Mercado no identificado'}</span><span><PackageCheck /> Degustación utilizada</span></div>
          <small className="sale-location">{market ? `${market.district} · ${market.province} · ${market.department}` : consumo.marketId}</small>
          <div className="sale-card-footer"><small>{consumo.id}</small></div>
        </div>
        <div className="sale-admin-actions"><StatusPill status={consumo.status} /><Btn variant="danger" onClick={() => onDeleteConsumo(consumo)} testId={`button-delete-tasting-consumption-${consumo.id}`}><Trash2 /> Eliminar</Btn></div>
      </article>;
    })}</div> : <Empty title="Aún no hay degustaciones utilizadas" detail="Las cantidades declaradas por los promotores aparecerán aquí." />}</div></section>
  </section>;
}
...
function CoordinatorApp({ user, markets, users, clients, sales, inventory, movements, assignments, notify }: { user: AppUser; markets: Market[]; users: AppUser[]; clients: Client[]; sales: Sale[]; inventory: MarketInventory[]; movements: InventoryMovement[]; assignments: PromoterAssignment[]; notify: (message: string, error?: boolean) => void }) {
 ...
 function AnalystApp({ user, markets, setMarkets, users, setUsers, clients, setClients, sales, setSales, attendance, setAttendance, inventory, movements, assignments, setAssignments, setInventory, setMovements, closures, setClosures, notify }: { user: AppUser; markets: Market[]; setMarkets: (value: Market[]) => void; users: AppUser[]; setUsers: (value: AppUser[]) => void; clients: Client[]; setClients: (value: Client[]) => void; sales: Sale[]; setSales: (value: Sale[]) => void; attendance: Attendance[]; setAttendance: (value: Attendance[]) => void; inventory: MarketInventory[]; movements: InventoryMovement[]; assignments: PromoterAssignment[]; setAssignments: (value: PromoterAssignment[]) => void; setInventory: (value: MarketInventory[]) => void; setMovements: (value: InventoryMovement[]) => void; closures: SessionClosure[]; setClosures: (value: SessionClosure[]) => void; notify: (message: string, error?: boolean) => void }) {
   const [tab, setTab] = useState('inicio'); const [query, setQuery] = useState(''); const [modal, setModal] = useState<'user' | 'client' | 'market' | 'canje' | null>(null); const [editingSale, setEditingSale] = useState<Sale | null>(null); const [syncing, setSyncing] = useState(false);
  ...
  return <main className="workspace">
    ...
    <nav className="tabs" aria-label="Módulos">{tabs.map(([value, label]) => <button key={value} className={`tab ${tab === value ? 'active' : ''}`} onClick={() => setTab(value)} data-testid={`tab-${value}`}>{label}</button>)}</nav>
      ...
      {tab === 'asignaciones' && <AssignmentModule markets={markets} users={users} clients={clients} assignments={assignments} setAssignments={setAssignments} setUsers={setUsers} notify={notify} />}
        {tab === 'ventas' && <section className="panel"><div className="panel-header"><div><h2>Ventas y evidencias</h2><p>Ordenadas desde la venta más reciente hasta la más antigua.</p></div><Btn variant="outline" onClick={exportSales}><Download /> Descargar</Btn></div><div className="panel-body sales-panel-body">{sales.length ? <div className="record-list sales-record-list">{sales.map(sale => {
          const promoter = users.find(item => item.id === sale.promoterId);
          const stock = userRedemptionStock(promoter || { id: '', dni: '', name: '', role: 'PROMOTOR', status: 'ACTIVO' });
          return <article className="record sale-admin-record" key={sale.id}>
            <span className="record-icon"><ShoppingBag /></span>
            <div className="record-main sale-card-main">
              <div className="sale-card-heading"><div><strong>{clients.find(client => client.id === sale.clientId)?.name || 'Tienda'}</strong><small>{promoter?.name || 'Promotor no identificado'} · DNI {promoter?.dni || '—'}</small></div><b>{formatSoles(sale.amountSoles)}</b></div>
              <div className="sale-card-meta"><span><Clock3 /> {formatDate(sale.date)}</span><span><MapPin /> {marketMap[sale.marketId]?.name || 'Mercado no identificado'}</span><span><PackageCheck /> {sale.units} unidades · {sale.mode}</span></div>
              <small className="sale-location">{saleMarketLocationText(sale, markets)} · Rol: {sale.promoterRoleLabel || promoter?.roleLabel || sale.promoterRole || promoter?.role || 'No identificado'}</small>
              <div className="sale-card-detail"><span><Gift /> {sale.bonus ? `${sale.redemptionCount ?? 1} canje${(sale.redemptionCount ?? 1) === 1 ? '' : 's'} · ${sale.bonus}` : 'Sin canje'}</span><span><PackageCheck /> Stock: {userTastingStock(promoter || { id: '', dni: '', name: '', role: 'PROMOTOR', status: 'ACTIVO' })} degustación · {sumRedemptionStock(stock)} canjes</span></div>
              {sale.comment && <p className="sale-comment">{sale.comment}</p>}
              <div className="sale-card-footer"><small>{sale.id}</small><div className="record-photos"><PhotoThumbnail label="Boleta" src={sale.receiptPhoto} />{sale.exchangePhoto && <PhotoThumbnail label="Canje" src={sale.exchangePhoto} />}</div></div>
            </div>
            <div className="sale-admin-actions"><StatusPill status={sale.status} /><Btn variant="outline" onClick={() => setEditingSale(sale)} testId={`button-edit-sale-${sale.id}`}><Pencil /> Editar</Btn><Btn variant="danger" onClick={() => deleteSale(sale)} testId={`button-delete-sale-${sale.id}`}><Trash2 /> Eliminar</Btn></div>
          </article>;
        })}</div> : <Empty />}</div></section>}
       {tab === 'marcaciones' && <><section className="panel"><div className="panel-header"><div><h2>Marcaciones de asistencia</h2><p>Ordenadas desde la más reciente hasta la más antigua.</p></div><Btn variant="outline" onClick={exportAttendance}><Download /> Descargar</Btn></div><div className="panel-body">{orderedAttendance.length ? <div className="record-list">{orderedAttendance.map(item => { const promoter = users.find(user => user.id === item.promoterId); const market = marketMap[item.marketId]; return <article className="record" key={item.id}><span className="record-icon"><Clock3 /></span><div className="record-main"><strong>{clients.find(client => client.id === item.clientId)?.name || 'Tienda'}</strong><small>{promoter?.name || 'Promotor no identificado'} · Rol: {item.promoterRole || promoter?.role || 'Rol no identificado'} · {formatDate(item.date)}</small><small><MapPin /> Departamento: {market?.department || 'No identificado'}</small><em><Camera /> {item.photo}</em><div className="record-photos"><PhotoThumbnail label={item.type === 'ENTRADA' ? 'Entrada' : 'Salida'} src={item.photo} /></div></div><span className={`status ${item.type === 'ENTRADA' ? 'active' : 'pending'}`}>{item.type}</span></article>; })}</div> : <Empty title="Aún no hay marcaciones" detail="Las entradas y salidas aparecerán aquí." />}</div></section>{automaticClosures.length > 0 && <section className="panel"><div className="panel-header"><div><h2>Evidencia de cierres automáticos</h2><p>Generados a las 11:59 p. m. cuando faltó el cierre manual.</p></div></div><div className="panel-body"><div className="record-list">{automaticClosures.map(closure => { const promoter = users.find(current => current.id === closure.promoterId); const market = marketMap[closure.marketId]; const client = clients.find(current => current.id === closure.clientId); return <article className="record" key={closure.id}><span className="record-icon"><ShieldCheck /></span><div className="record-main"><strong>{client?.name || 'Cliente no identificado'}</strong><small>{promoter?.name || 'Promotor no identificado'} · {formatDate(closure.date)}</small><small><MapPin /> {market?.name || 'Mercado no identificado'}</small><em>{closure.evidence || 'Cierre automático generado por el sistema.'}</em></div><span className="status pending">AUTOMÁTICO</span></article>; })}</div></div></section>}</>}
