...
type Sale = { id: string; promoterId: string; clientId: string; marketId: string; mode: 'UNIDADES' | 'PLANCHAS'; units: number; amountSoles: number; planchas?: number; mix: Record<string, number>; bonus?: string; comment?: string; receiptPhoto: string; exchangePhoto?: string; date: string; status: SyncStatus };
...
function AnalystApp({ user, markets, setMarkets, users, setUsers, clients, setClients, sales, attendance, inventory, movements, setInventory, setMovements, notify }: { user: AppUser; markets: Market[]; setMarkets: (value: Market[]) => void; users: AppUser[]; setUsers: (value: AppUser[]) => void; clients: Client[]; setClients: (value: Client[]) => void; sales: Sale[]; attendance: Attendance[]; inventory: MarketInventory[]; movements: InventoryMovement[]; setInventory: (value: MarketInventory[]) => void; setMovements: (value: InventoryMovement[]) => void; notify: (message: string, error?: boolean) => void }) {
  ...
  const exportSales = () => { downloadCsv(`ventas-${today}.csv`, ['Código venta', 'Fecha', 'Promotor', 'Cliente', 'Mercado', 'Tipo', 'Unidades', 'Ingreso (S/)', 'Bonificación', 'Comentario', 'Estado'], sales.map(sale => [sale.id, formatDate(sale.date), users.find(user => user.id === sale.promoterId)?.name, clients.find(client => client.id === sale.clientId)?.name, marketMap[sale.marketId]?.name, sale.mode, sale.units, (sale.amountSoles ?? 0).toFixed(2), sale.bonus || 'Sin canje', sale.comment || '', sale.status])); notify('Reporte de ventas descargado'); };
...
function PromoterApp({ user, markets, clients, sales, setSales, attendance, setAttendance, inventory, setInventory, movements, setMovements, notify, onSessionSelection }: { user: AppUser; markets: Market[]; clients: Client[]; sales: Sale[]; setSales: (value: Sale[]) => void; attendance: Attendance[]; setAttendance: (value: Attendance[]) => void; inventory: MarketInventory[]; setInventory: (value: MarketInventory[]) => void; movements: InventoryMovement[]; setMovements: (value: InventoryMovement[]) => void; notify: (message: string, error?: boolean) => void; onSessionSelection: (selection: { marketId: string; clientId: string }) => void }) {
  ...
  const saveSale = () => {
     ...
     const next = [sale, ...sales]; setSales(next); writeStore('bt-sales', next);
      ...
      setAmountSoles(''); setComment(''); setReceipt(null); setExchange(null); setView('LISTA'); notify(bonus ? 'Venta y canje registrados. Stock actualizado.' : 'Venta sin canje registrada');
  };
