...
function AssignmentModule({ markets, users, clients, assignments, setAssignments, setUsers, notify }: { markets: Market[]; users: AppUser[]; clients: Client[]; assignments: PromoterAssignment[]; setAssignments: (value: PromoterAssignment[]) => void; setUsers: (value: AppUser[]) => void; notify: (message: string, error?: boolean) => void }) {
  ...
  return <section className="assignment-module">
    ...
    <div className="assignment-grid">
      <section className="panel assignment-card"><div className="panel-header"><div><h2>1. Elige un promotor</h2><p>{promoters.length} promotores activos disponibles.</p></div><Users /></div><div className="panel-body"><SelectField label="Promotor *" value={selectedPromoterId} onChange={selectPromoter} items={promoters.map(promoter => ({ value: promoter.id, label: `${promoter.name} · DNI ${promoter.dni}` }))} placeholder="Seleccionar promotor" />{selectedPromoter && <div className="assignment-person"><span className="record-icon"><UserRound /></span><div><strong>{selectedPromoter.name}</strong><small>{selectedPromoter.dni} · {selectedPromoter.role}</small></div><StatusPill status={selectedPromoter.status} /></div>}<div className="assignment-subtitle"><strong>Mercados asignados</strong><small>Puede trabajar en uno o varios mercados.</small></div>{activeMarkets.length ? <details className="assignment-dropdown"><summary><span>Seleccionar mercados</span><strong>{selectedMarketIds.length ? `${selectedMarketIds.length} mercado${selectedMarketIds.length === 1 ? '' : 's'} seleccionado${selectedMarketIds.length === 1 ? '' : 's'}` : 'Ningún mercado seleccionado'}</strong></summary><div className="assignment-dropdown-menu"><div className="assignment-dropdown-head"><small>{activeMarkets.length} mercados activos</small><button type="button" className="text-button" onClick={() => setSelectedMarketIds(activeMarkets.map(market => market.id))}>Seleccionar todos</button></div><div className="assignment-check-list">{activeMarkets.map(market => <label className={`assignment-check ${selectedMarketIds.includes(market.id) ? 'selected' : ''}`} key={market.id}><input type="checkbox" checked={selectedMarketIds.includes(market.id)} onChange={() => toggleMarket(market.id)} /><span><strong>{market.name}</strong><small>{market.region || market.department} · {market.district}</small></span></label>)}</div></div></details> : <Empty title="No hay mercados activos" detail="Actualiza primero la hoja de Mercados." />}</div></section>
