...
type Sale = { id: string; promoterId: string; clientId: string; marketId: string; mode: 'UNIDADES' | 'PLANCHAS'; units: number; amountSoles: number; planchas?: number; mix: Record<string, number>; bonus?: string; receiptPhoto: string; exchangePhoto?: string; date: string; status: 'SINCRONIZADA' | 'PENDIENTE' };
...
function PromoterApp({ user, market, clients, sales, setSales, attendance, setAttendance, notify }: { user: AppUser; market?: Market; clients: Client[]; sales: Sale[]; setSales: (value: Sale[]) => void; attendance: Attendance[]; setAttendance: (value: Attendance[]) => void; notify: (message: string, error?: boolean) => void }) {
  ...
  const [markClientId, setMarkClientId] = useState(available[0]?.id || ''); const [markType, setMarkType] = useState<'ENTRADA' | 'SALIDA'>('ENTRADA'); const [markPhoto, setMarkPhoto] = useState<File | null>(null); const [search, setSearch] = useState(''); const [modeFilter, setModeFilter] = useState<'TODO' | 'UNIDADES' | 'PLANCHAS'>('TODO');
   const totalMix = mix.TODINNO + mix.COSTA + mix.PASQUALINO; const total = mode === 'UNIDADES' ? unitQty : totalMix; const saleAmount = parseSoles(amountSoles); const bonus = bonusFor(mode, total, planchas); const validMix = mode === 'UNIDADES' || (totalMix === planchas * 6 && Object.values(mix).every(value => value >= 1)); const canConfirm = Boolean(clientId && receipt && Number.isFinite(saleAmount) && saleAmount > 0 && validMix && (!bonus || exchange) && !(mode === 'PLANCHAS' && planchas > 80));
  ...
  const saveSale = () => {
     if (!canConfirm) { notify(mode === 'PLANCHAS' && planchas > 80 ? 'Requiere autorización previa de Trade' : Number.isFinite(saleAmount) && saleAmount > 0 ? 'Completa venta y evidencias' : 'Ingresa un monto mayor a S/ 0.00', true); return; }
     ...
     const next = [sale, ...sales]; setSales(next); writeStore('bt-sales', next); setAmountSoles(''); setReceipt(null); setExchange(null); setView('LISTA'); notify(bonus ? 'Venta y canje registrados' : 'Venta sin canje registrada');
  };
