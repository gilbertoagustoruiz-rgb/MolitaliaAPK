...
function SalesDashboard({ sales, markets, users, onViewSales, onExport }: { sales: Sale[]; markets: Market[]; users: AppUser[]; onViewSales: () => void; onExport: () => void }) {
  ...
  const stockRows = useMemo(() => users.filter(user => isPromoterRole(user.role) && user.status === 'ACTIVO').map(promoter => {
    const tastingStock = userTastingStock(promoter);
    const redemptionStock = userRedemptionStock(promoter);
...
function PromoterApp({ user, markets, clients, assignments, productPrices, sales, setSales, attendance, setAttendance, inventory, setInventory, movements, setMovements, notify, onSessionSelection, onUpdateUser, onLogout, enqueueEvidencePhoto }: { user: AppUser; markets: Market[]; clients: Client[]; assignments: PromoterAssignment[]; productPrices: ProductPrice[]; sales: Sale[]; setSales: (value: Sale[]) => void; attendance: Attendance[]; setAttendance: (value: Attendance[]) => void; inventory: MarketInventory[]; setInventory: (value: MarketInventory[]) => void; movements: InventoryMovement[]; setMovements: (value: InventoryMovement[]) => void; notify: (message: string, error?: boolean) => void; onSessionSelection: (selection: { marketId: string; clientId: string }) => void; onUpdateUser: (user: AppUser) => void; onLogout: () => void; enqueueEvidencePhoto: (file: File, entityType: PendingPhotoUpload['entityType'], entityId: string, field: PendingPhotoUpload['field'], context: PhotoUploadContext) => void }) {
    ...
    const mineSales = sales.filter(sale => sale.promoterId === user.id); const mineAttendance = attendance.filter(item => item.promoterId === user.id); const today = new Date().toISOString().slice(0, 10); const todayAttendance = mineAttendance.filter(item => item.date.slice(0, 10) === today); const todayAttendanceCount = (type: 'ENTRADA' | 'SALIDA') => todayAttendance.filter(item => item.type === type).length; const attendanceLimitReached = (type: 'ENTRADA' | 'SALIDA') => todayAttendanceCount(type) >= MAX_DAILY_ATTENDANCE_BY_TYPE; const todayAttendanceFor = (id: string) => mineAttendance.filter(item => item.clientId === id && item.date.slice(0, 10) === today); const latestAttendanceFor = (id: string) => todayAttendanceFor(id).sort((first, second) => new Date(second.date).getTime() - new Date(first.date).getTime())[0]; const exitedToday = (id: string) => todayAttendanceFor(id).some(item => item.type === 'SALIDA'); const sellingClients = available.filter(client => latestAttendanceFor(client.id)?.type === 'ENTRADA' && !exitedToday(client.id)); const canSellForSelectedClient = Boolean(clientId && latestAttendanceFor(clientId)?.type === 'ENTRADA' && !exitedToday(clientId)); const canConfirm = Boolean(canSellForSelectedClient && saleFormValid); const filteredSales = mineSales.filter(sale => (modeFilter === 'TODO' || sale.mode === modeFilter) && `${sale.id} ${clients.find(client => client.id === sale.clientId)?.name || ''} ${sale.bonus || ''} ${sale.comment || ''}`.toLowerCase().includes(search.toLowerCase())); const filteredAttendance = mineAttendance.filter(item => `${clients.find(client => client.id === item.clientId)?.name || ''} ${item.type}`.toLowerCase().includes(search.toLowerCase()));
  const saveSale = () => {
