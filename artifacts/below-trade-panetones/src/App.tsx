...
function InventoryModule({ markets, inventory, movements, actor, setInventory, setMovements, notify }: { markets: Market[]; inventory: MarketInventory[]; movements: InventoryMovement[]; actor: AppUser; setInventory: (value: MarketInventory[]) => void; setMovements: (value: InventoryMovement[]) => void; notify: (message: string, error?: boolean) => void }) {
  ...
  useEffect(() => {
    setDrafts(current => {
      ...
      markets.forEach(market => {
        ...
        if (!next[market.id]) next[market.id] = { tasting: String(stock.tastingStock), redemption: redemptionItems.reduce((result, item) => ({ ...result, [item.id]: String(stock.redemptionStock[item.id]) }), {} as Record<RedemptionItemId, string>) };
      });
      return next;
    });
  }, [markets, inventory]);
  const saveStock = (market: Market) => {
    ...
    const draft = drafts[market.id] || { tasting: String(current.tastingStock), redemption: redemptionItems.reduce((result, item) => ({ ...result, [item.id]: String(current.redemptionStock[item.id]) }), {} as Record<RedemptionItemId, string>) };
    const tasting = Number(draft.tasting);
    const redemptionStock = redemptionItems.reduce((result, item) => ({ ...result, [item.id]: Number(draft.redemption[item.id]) }), {} as RedemptionStock);
    if (!Number.isInteger(tasting) || tasting < 0 || redemptionItems.some(item => !Number.isInteger(redemptionStock[item.id]) || redemptionStock[item.id] < 0)) { notify('El stock debe ser un número entero igual o mayor a 0', true); return; }
    const tastingDelta = tasting - current.tastingStock; const updatedAt = new Date().toISOString();
    const nextInventory = inventory.some(item => item.marketId === market.id)
      ? inventory.map(item => item.marketId === market.id ? { ...item, tastingStock: tasting, redemptionStock, updatedAt } : item)
      : [...inventory, { marketId: market.id, tastingStock: tasting, redemptionStock, updatedAt }];
    ...
    redemptionItems.forEach(item => {
      const delta = redemptionStock[item.id] - current.redemptionStock[item.id];
      if (delta !== 0) newMovements.unshift({ id: `INV-${Date.now()}-${item.id}`, marketId: market.id, kind: 'AJUSTE_CANJES', itemId: item.id, quantity: delta, actorId: actor.id, actorName: actor.name, date: updatedAt, status: syncStatus() });
    });
    ...
    notify(tastingDelta || redemptionItems.some(item => redemptionStock[item.id] !== current.redemptionStock[item.id]) ? `Stock actualizado para ${market.name}` : 'No hubo cambios en el stock');
  };
  ...
  return <section className="inventory-module">
    ...
    <div className="inventory-grid">{markets.map(market => {
      ...
      return <article className="inventory-card" key={market.id}><div className="inventory-card-head"><div><span className="inventory-market-id">{market.id}</span><h3>{market.name}</h3><p>{market.district} · {market.province}</p></div><span className={`stock-level ${levelClass(minimumStock)}`}>{minimumStock === 0 ? 'Agotado' : minimumStock <= 5 ? 'Stock bajo' : 'Disponible'}</span></div><div className="stock-counters"><div><small>DEGUSTACIÓN</small><strong>{stock.tastingStock}</strong><span>panetones</span></div><div><small>CANJES</small><strong>{totalRedemption}</strong><span>premios</span></div></div><div className="stock-inputs"><Field label="Actualizar degustación"><Input type="number" min={0} step={1} value={draft.tasting} onChange={value => setDrafts({ ...drafts, [market.id]: { ...draft, tasting: value } })} testId={`input-stock-tasting-${market.id}`} /></Field></div><div className="redemption-inputs"><p>Premios disponibles para canje</p>{redemptionItems.map(item => <Field label={item.label} key={item.id}><Input type="number" min={0} step={1} value={draft.redemption[item.id]} onChange={value => setDrafts({ ...drafts, [market.id]: { ...draft, redemption: { ...draft.redemption, [item.id]: value } } })} testId={`input-stock-${item.id.toLowerCase()}-${market.id}`} /></Field>)}</div><Btn onClick={() => saveStock(market)} testId={`button-save-stock-${market.id}`}><RefreshCw /> Guardar stock</Btn></article>;
    })}</div>
...
function PromoterApp({ user, market, clients, sales, setSales, attendance, setAttendance, inventory, setInventory, movements, setMovements, notify }: { user: AppUser; market?: Market; clients: Client[]; sales: Sale[]; setSales: (value: Sale[]) => void; attendance: Attendance[]; setAttendance: (value: Attendance[]) => void; inventory: MarketInventory[]; setInventory: (value: MarketInventory[]) => void; movements: InventoryMovement[]; setMovements: (value: InventoryMovement[]) => void; notify: (message: string, error?: boolean) => void }) {
   ...
   const [clientId, setClientId] = useState(available[0]?.id || ''); const [mode, setMode] = useState<'UNIDADES' | 'PLANCHAS'>('UNIDADES'); const [sku, setSku] = useState(products[0].sku); const [unitQty, setUnitQty] = useState(1); const [amountSoles, setAmountSoles] = useState(''); const [planchas, setPlanchas] = useState(1); const [mix, setMix] = useState({ TODINNO: 1, COSTA: 1, PASQUALINO: 4 }); const [receipt, setReceipt] = useState<File | null>(null); const [exchange, setExchange] = useState<File | null>(null);
    const [markClientId, setMarkClientId] = useState(available[0]?.id || ''); const [markType, setMarkType] = useState<'ENTRADA' | 'SALIDA'>('ENTRADA'); const [markPhoto, setMarkPhoto] = useState<File | null>(null); const [tastingPrompt, setTastingPrompt] = useState(false); const [search, setSearch] = useState(''); const [modeFilter, setModeFilter] = useState<'TODO' | 'UNIDADES' | 'PLANCHAS'>('TODO');
    const marketStock = inventory.find(item => item.marketId === user.marketId) || emptyInventory(user.marketId || ''); const redemptionTotal = redemptionItems.reduce((sum, item) => sum + marketStock.redemptionStock[item.id], 0); const totalMix = mix.TODINNO + mix.COSTA + mix.PASQUALINO; const total = mode === 'UNIDADES' ? unitQty : totalMix; const saleAmount = parseSoles(amountSoles); const bonus = bonusFor(mode, total, planchas); const requiredRedemptions = parseBonusItems(bonus); const missingRedemption = requiredRedemptionEntries(requiredRedemptions).find(([itemId, quantity]) => marketStock.redemptionStock[itemId] < quantity); const validMix = mode === 'UNIDADES' || (totalMix === planchas * 6 && Object.values(mix).every(value => value >= 1)); const canConfirm = Boolean(clientId && receipt && Number.isFinite(saleAmount) && saleAmount > 0 && validMix && (!bonus || (exchange && !missingRedemption)) && !(mode === 'PLANCHAS' && planchas > 80));
  ...
  const saveSale = () => {
     ...
     const next = [sale, ...sales]; setSales(next); writeStore('bt-sales', next);
      if (bonus) {
        const nextInventory = inventory.map(item => item.marketId === user.marketId ? { ...item, redemptionStock: requiredRedemptionEntries(requiredRedemptions).reduce((nextStock, [itemId, quantity]) => ({ ...nextStock, [itemId]: nextStock[itemId] - quantity }), { ...item.redemptionStock }), updatedAt: now } : item);
       const currentMovements = readStore<InventoryMovement[]>('bt-inventory-movements', []);
        const canjeMovements = requiredRedemptionEntries(requiredRedemptions).map(([itemId, quantity]) => ({ id: `CAN-${id}-${itemId}`, marketId: user.marketId || '', kind: 'CANJE' as const, itemId, quantity, actorId: user.id, actorName: user.name, date: now, status: syncStatus() }));
   ...
   const finalizeAttendance = (tastingUsed = 0) => {
     const stock = inventory.find(item => item.marketId === user.marketId) || emptyInventory(user.marketId || '');
     if (tastingUsed > stock.tastingStock) { notify(`Solo hay ${stock.tastingStock} panetones de degustación disponibles`, true); return; }
     const now = new Date(); const id = `MAR-${now.getFullYear()}-${String(attendance.length + 1).padStart(6, '0')}`; const type = markType; const item: Attendance = { id, promoterId: user.id, clientId: markClientId, marketId: user.marketId || '', type, photo: `${type} - ${available.find(client => client.id === markClientId)?.name || 'TIENDA'} - ${id}.jpg`, date: now.toISOString(), status: syncStatus() };
     const next = [item, ...attendance]; setAttendance(next); writeStore('bt-attendance', next);
     if (type === 'SALIDA') {
       const updatedInventory = inventory.some(entry => entry.marketId === user.marketId)
         ? inventory.map(entry => entry.marketId === user.marketId ? { ...entry, tastingStock: entry.tastingStock - tastingUsed, updatedAt: now.toISOString() } : entry)
         : [...inventory, { ...stock, tastingStock: stock.tastingStock - tastingUsed, updatedAt: now.toISOString() }];
       const movement: InventoryMovement = { id: `DEG-${id}`, marketId: user.marketId || '', kind: 'DEGUSTACION', quantity: tastingUsed, actorId: user.id, actorName: user.name, date: now.toISOString(), status: syncStatus() };
  ...
  return <main className="promoter-page"><div className="promoter-layout"><PromoterNav active={module} onChange={value => { setModule(value); setView('LISTA'); setSearch(''); }} /><div className="promoter-content">
    <div className="promoter-head"><div><span className="crumb">INICIO / PROCESAMIENTO DE PEDIDOS / {module === 'VENTAS' ? 'VENTAS' : 'TURNOS Y ASISTENCIAS'}</span><h1>{module === 'VENTAS' ? (view === 'LISTA' ? <>Mis ventas <small>{mineSales.length} registros</small></> : 'Añadir venta') : (view === 'LISTA' ? 'Registros de marcación' : 'Añadir registro de marcación')}</h1><p>{market.name} · {user.name}</p></div>{view === 'LISTA' && <Btn onClick={() => setView('NUEVA')} testId={`button-new-${module.toLowerCase()}`}><Plus /> Nueva {module === 'VENTAS' ? 'venta' : 'marcación'}</Btn>}</div>
     <div className="market-banner"><span className="market-pin"><MapPin /></span><div><small>MERCADO ASIGNADO</small><strong>{market.name}</strong><p>{market.district} · {market.province} · {market.department}</p></div><div className="market-stock-mini"><span>Premios <strong>{redemptionTotal}</strong></span><span>Degustación <strong>{marketStock.tastingStock}</strong></span></div><StatusPill status="ACTIVO" /></div>
...
export default function App() {
  ...
  return <>{activeUser ? <Shell user={activeUser} logout={logout}>{activeUser.role === 'PROMOTOR' ? <PromoterApp user={activeUser} market={markets.find(market => market.id === activeUser.marketId)} clients={clients} sales={sales} setSales={setSales} attendance={attendance} setAttendance={setAttendance} inventory={inventory} setInventory={setInventory} movements={movements} setMovements={setMovements} notify={notify} /> : <AnalystApp user={activeUser} markets={markets} setMarkets={setMarkets} users={users} setUsers={setUsers} clients={clients} setClients={setClients} sales={sales} attendance={attendance} inventory={inventory} movements={movements} setInventory={setInventory} setMovements={setMovements} notify={notify} />}</Shell> : <Login onLogin={setUser} notify={notify} />}<ToastView toast={toast} clear={() => setToast(null)} /></>;
}