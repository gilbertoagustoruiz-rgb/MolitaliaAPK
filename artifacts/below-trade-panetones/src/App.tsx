...
function NewUserModal({ markets, clients, onSave, close }: { markets: { value: string; label: string }[]; clients: Client[]; onSave: (user: AppUser) => void; close: () => void }) {
  ...
  return <Modal title="Crear usuario" detail="Define acceso, rol y mercado o cliente." close={close}><div className="form-grid"><Field label="DNI *"><Input value={dni} onChange={value => setDni(value.replace(/\D/g, ''))} maxLength={8} testId="input-new-user-dni" /></Field><Field label="Nombre completo *"><Input value={name} onChange={setName} testId="input-new-user-name" /></Field><SelectField label="Rol *" value={role} onChange={value => setRole(value as Role)} items={[...promoterRoles, 'SUPERVISOR', 'ANALISTA', 'TRADE', 'ADMIN', 'CLIENTE'].map(value => ({ value, label: value }))} />{isPromoterRole(role) && <SelectField label="Mercado asignado *" value={marketId} onChange={setMarketId} items={markets} />}{role === 'CLIENTE' && <SelectField label="Cliente vinculado *" value={clientId} onChange={setClientId} items={clients.filter(client => client.status === 'ACTIVO').map(client => ({ value: client.id, label: `${client.code} · ${client.name}` }))} placeholder="Seleccionar cliente" />}<Field label="Clave temporal *"><Input value={password} onChange={value => setPassword(value)} type="password" testId="input-new-user-password" /></Field></div><div className="modal-actions"><Btn variant="outline" onClick={close}>Cancelar</Btn><Btn onClick={save} testId="button-create-user">Crear usuario</Btn></div></Modal>;
}
