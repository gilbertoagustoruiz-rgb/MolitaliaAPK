...
async function uploadEvidenceFile(file: File, entityType: PendingPhotoUpload['entityType'], field: PendingPhotoUpload['field'], context: PhotoUploadContext) {
  ...
  return uploadPhoto({
    ...
    entityType,
    entityId: `${entityType}-${crypto.randomUUID()}`,
    field,
    ...context,
...
function PhotoField({ label, hint, file, setFile, disabled = false, uploadedUrl, uploading = false }: { label: string; hint: string; file: File | null; setFile: (file: File | null) => void; disabled?: boolean; uploadedUrl?: string; uploading?: boolean }) {
  return <label className={`photo-field ${uploadedUrl ? 'ready' : file ? 'pending' : ''} ${disabled ? 'disabled' : ''}`} data-testid={`photo-field-${label.toLowerCase().replaceAll(' ', '-')}`}>
    ...
    {uploadedUrl ? <img src={photoImageSource(uploadedUrl)} alt={label} loading="lazy" /> : <span><Camera /></span>}
