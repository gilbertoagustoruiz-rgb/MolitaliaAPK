...
function AnalystApp({ user, markets, setMarkets, users, setUsers, clients, setClients, sales, attendance, inventory, movements, setInventory, setMovements, notify }: { user: AppUser; markets: Market[]; setMarkets: (value: Market[]) => void; users: AppUser[]; setUsers: (value: AppUser[]) => void; clients: Client[]; setClients: (value: Client[]) => void; sales: Sale[]; attendance: Attendance[]; inventory: MarketInventory[]; movements: InventoryMovement[]; setInventory: (value: MarketInventory[]) => void; setMovements: (value: InventoryMovement[]) => void; notify: (message: string, error?: boolean) => void }) {
  ...
  const importMarkets = async () => {
    ...
    try {
      ...
      const lines = (await response.text()).split(/\r?\n/).filter(Boolean); const imported = lines.slice(1).map((line, index) => { const cells = parseCsvLine(line); const department = (cells[0] || 'LIMA').toUpperCase(); const province = (cells[1] || 'LIMA').toUpperCase(); const district = (cells[2] || 'LIMA').toUpperCase(); const name = (cells[3] || cells[2] || 'MERCADO IMPORTADO').toUpperCase(); const existing = markets.find(market => market.name === name && market.district === district && market.province === province); return { id: existing?.id || `SHEET-${index + 1}`, department, province, district, name, status: 'ACTIVO' as Status }; }).filter(market => market.name);
      if (!imported.length) throw new Error('La hoja no contiene mercados');
      const importedIds = new Set(imported.map(market => market.id)); const referencedIds = new Set([...users.map(item => item.marketId), ...clients.map(item => item.marketId), ...inventory.map(item => item.marketId)].filter(Boolean) as string[]);
      const preservedAssignments = markets.filter(market => referencedIds.has(market.id) && !importedIds.has(market.id));
      const nextMarkets = [...imported, ...preservedAssignments]; setMarkets(nextMarkets); writeStore('bt-markets', nextMarkets); notify(`${imported.length} mercados importados desde Google Sheets`);
